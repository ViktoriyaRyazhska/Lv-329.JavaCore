...
public class Programer extends Stuff {
	...
	private void inputHour(Scanner sc) {
	...
	private void inputRank(Scanner sc) {
		System.out.println("Enter number of Programmer's rank (Junior - 1, Medium - 2, Senior - 3):");
		String ranktemp = sc.next();
		Pattern rankpat = Pattern.compile("[1-3]{1}");
		Matcher rankmat = rankpat.matcher(ranktemp);
		if (rankmat.matches()) {
			switch (Integer.parseInt(ranktemp)) {
			case 1:
